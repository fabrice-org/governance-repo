...
async function refresh (robot, opts = {}) {
  ...
  inFlightPromise = (async () => {
    try {
      robot.log && robot.log.debug && robot.log.debug('Installation cache: refreshing installations from GitHub')
      ...
      robot.log && robot.log.debug && robot.log.debug(`Installation cache: refreshed ${cachedInstallations.length} install(s), orgs=[${cachedOrgLogins.join(', ')}]`)
    } catch (e) {
